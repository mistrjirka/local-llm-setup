...
The real full `AD-Q6_K-Q5_K` GGUF is validated with this production geometry, including the CPU vision projector. In fixed-350k mode all four slots report `n_ctx=350000` with MTP enabled. The MTP draft is deliberately placed on `CUDA1`; placing it on the V100 crosses the V100 allocation boundary.

A near-limit real-model benchmark at **348744 cached + 1000 prompt + 256 generated** measured **388.91 PP/s** (2.571 s for the 1k append) and **33.11 TG/s** with MTP3 (150/312 drafts accepted). The previous `AD-Q5_K-Q4_K` model under the same geometry measured **385.62 PP/s / 33.26 TG/s** (149/315 accepted), so moving to `AD-Q6_K-Q5_K` was **+0.85% PP / -0.45% TG** in this test while improving quantization fidelity. The 348744-token one-time Q6/Q5 cache construction averaged **776.60 PP/s**; its deferred MTP catch-up is persisted by the `.draft`/`.spec` slot companions.
